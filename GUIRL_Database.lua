GUIRL = GUIRL or {}
CUOR = GUIRL

GUIRL.Consumables = {
    {
        label = "Blinding Light",
        itemID = 22861,
        enabled = true,
    },
    {
        label = "Fortification",
        itemID = 22851,
        enabled = true,
    },
    {
        label = "Mighty Restoration",
        itemID = 22853,
        enabled = true,
    },
    {
        label = "Pure Death",
        itemID = 22866,
        enabled = true,
    },
    {
        label = "Relentless Assault",
        itemID = 22854,
        enabled = true,
    },
    {
        label = "Supreme Power",
        itemID = 13512,
        enabled = true,
    },
    {
        label = "Adept's Elixir",
        itemID = 28103,
        enabled = true,
    },
    {
        label = "Demonslaying",
        itemID = 9224,
        enabled = true,
    },
    {
        label = "Draenic Wisdom",
        itemID = 32067,
        enabled = true,
    },
    {
        label = "Major Agility",
        itemID = 22831,
        enabled = true,
    },
    {
        label = "Major Firepower",
        itemID = 22833,
        enabled = true,
    },
    {
        label = "Major Frost Power",
        itemID = 22834,
        enabled = true,
    },
    {
        label = "Major Shadow Power",
        itemID = 22835,
        enabled = true,
    },
    {
        label = "Mastery",
        itemID = 28104,
        enabled = true,
    },
    {
        label = "Major Onslaught",
        itemID = 22832,
        enabled = true,
    },
    {
        label = "Healing Power",
        itemID = 22825,
        enabled = true,
    },
    {
        label = "Major Defense",
        itemID = 22840,
        enabled = true,
    },
    {
        label = "Major Mageblood",
        itemID = 22841,
        enabled = true,
    },
    {
        label = "Major Fortitude",
        itemID = 22828,
        enabled = true,
    },
    {
        label = "Ironskin",
        itemID = 32068,
        enabled = true,
    },
    {
        label = "Major Strength",
        itemID = 22824,
        enabled = true,
    },
    {
        label = "Major Accuracy",
        itemID = 22826,
        enabled = true,
    },
    {
        label = "Destruction Potion",
        itemID = 22839,
        enabled = true,
    },
    {
        label = "Haste Potion",
        itemID = 22838,
        enabled = true,
    },
    {
        label = "Ironshield Potion",
        itemID = 22849,
        enabled = true,
    },
    {
        label = "Major Dreamless Sleep Potion",
        itemID = 22836,
        enabled = true,
    },
    {
        label = "Mana Potion",
        itemID = 22832,
        enabled = true,
    },
    {
        label = "Healing Potion",
        itemID = 22829,
        enabled = true,
    },
    {
        label = "Heroic Potion",
        itemID = 22837,
        enabled = true,
    },
    {
        label = "Mad Alchemist's Potion",
        itemID = 34440,
        enabled = true,
    },
    {
        label = "Nightmare Seed",
        itemID = 22797,
        enabled = true,
    },
    {
        label = "Flame Cap",
        itemID = 22788,
        enabled = true,
    },
    {
        label = "Superior Wizard Oil",
        itemID = 22522,
        enabled = true,
    },
    {
        label = "Brilliant Wizard Oil",
        itemID = 20749,
        enabled = true,
    },
    {
        label = "Superior Mana Oil",
        itemID = 22521,
        enabled = true,
    },
    {
        label = "Brilliant Mana Oil",
        itemID = 20748,
        enabled = true,
    },
    {
        label = "Adamantite Sharpening Stone",
        itemID = 23529,
        enabled = true,
    },
    {
        label = "Fel Sharpening Stone",
        itemID = 23528,
        enabled = true,
    },
    {
        label = "Adamantite Weightstone",
        itemID = 28421,
        enabled = true,
    },
    {
        label = "Fel Weightstone",
        itemID = 23559,
        enabled = true,
    },
    {
        label = "Blackened Basilisk",
        itemID = 27657,
        enabled = true,
    },
    {
        label = "Grilled Mudfish",
        itemID = 27664,
        enabled = true,
    },
    {
        label = "Poached Bluefish",
        itemID = 27665,
        enabled = true,
    },
    {
        label = "Ravager Dog",
        itemID = 27655,
        enabled = true,
    },
    {
        label = "Roasted Clefthoof",
        itemID = 27658,
        enabled = true,
    },
    {
        label = "Skullfish Soup",
        itemID = 33825,
        enabled = true,
    },
    {
        label = "Warp Burger",
        itemID = 27660,
        enabled = true,
    },
    {
        label = "Spicy Crawdad",
        itemID = 33052,
        enabled = true,
    },
    {
        label = "Golden Fish Sticks",
        itemID = 27666,
        enabled = true,
    },
    {
        label = "Spicy Hot Talbuk",
        itemID = 33872,
        enabled = true,
    },
    {
        label = "Drums of Battle",
        itemID = 29529,
        enabled = true,
    },
    {
        label = "Drums of Restoration",
        itemID = 29531,
        enabled = true,
    },
    {
        label = "Drums of War",
        itemID = 29530,
        enabled = true,
    },
    {
        label = "Demonic Rune",
        itemID = 12662,
        enabled = true,
    },
    {
        label = "Dark Rune",
        itemID = 20520,
        enabled = true,
    },
    {
        label = "Agility V",
        itemID = 27498,
        enabled = true,
    },
    {
        label = "Strength V",
        itemID = 27503,
        enabled = true,
    },
    {
        label = "Stamina V",
        itemID = 27501,
        enabled = true,
    },
    {
        label = "Spirit V",
        itemID = 27504,
        enabled = true,
    },
    {
        label = "Intellect V",
        itemID = 27499,
        enabled = true,
    },
    {
        label = "Protection V",
        itemID = 27500,
        enabled = true,
    },
    {
        label = "Fel Blossom",
        itemID = 22795,
        enabled = true,
    },
    {
        label = "Bloodberry Elixir",
        itemID = 32721,
        enabled = true,
    },
    {
        label = "Arcane Protection Potion",
        itemID = 13461,
        enabled = true,
    },
    {
        label = "Fire Protection Potion",
        itemID = 13457,
        enabled = true,
    },
    {
        label = "Frost Protection Potion",
        itemID = 13456,
        enabled = true,
    },
    {
        label = "Nature Protection Potion",
        itemID = 13458,
        enabled = true,
    },
    {
        label = "Shadow Protection Potion",
        itemID = 13459,
        enabled = true,
    },
}

GUIRL.Settings = {
    priceSource = "AUCTIONATOR",
    autoResetOnRaidEnter = true,
    framePoint = "CENTER",
    frameX = 0,
    frameY = 0,
    frameWidth = 340,
    rowHeight = 18,
    title = "Gold used In Riad Loser",
}
